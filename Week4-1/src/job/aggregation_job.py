...
def create_processed_events_source_kafka(t_env):
    ...
    table_name = "w4_processed_events_kafka"
...
def create_sessionized_events_sink_postgres(t_env):
    table_name = 'w4_sessionized_events'
    sink_ddl = f"""
        CREATE TABLE {table_name} (
            ip VARCHAR,
            host VARCHAR,
...
def create_aggregated_events_sink_postgres(t_env):
    table_name = 'w4_aggregated_session_data'
    sink_ddl = f"""
        CREATE TABLE {table_name} (
            host VARCHAR,
...
def log_aggregation():
    ...
    try:
        ...
        sessionized_table = t_env.from_path(source_table) \
            ...
            .group_by(col("ip"), col("host"), col("session_window")) \
            .select(
                col("ip"),
                col("host"),
        ...
        # Write sessionized events to Postgres
        sessionized_table.execute_insert(session_sink_table)
        ...
        print("Aggregation job failed:", str(e))
